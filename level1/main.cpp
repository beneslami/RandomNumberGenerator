...
int main(int argc, char **argv){
    ...
    if(mkdir(temp_dir.append("synthetic/").c_str() , 0777) == -1){
        std::cerr << "Error creating directory\n";
        output = -1;
    }
    else{
        temp_dir = directory;
        std::fstream iatFile(temp_dir.append("synthetic/iat_sequence.txt"), std::ios::out);
        ...
        iatFile.close();
        temp_dir = directory;
        std::fstream intensityFile(temp_dir.append("synthetic/intensity_sequence.txt"), std::ios::out);
        ...
        intensityFile.close();
        temp_dir = directory;
        std::fstream temporalFile(temp_dir.append("synthetic/temporal_sequence.txt"), std::ios::out);
